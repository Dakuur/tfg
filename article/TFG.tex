...
\begin{enumerate}\itemsep1pt
  ...
  \item \textbf{Cerca bayesiana + CV 5-\textit{fold}} sobre el 85\% restant: cada configuració
        s'avalua amb \textit{Stratified K-Fold} per pacient, amb \textit{early stopping}
        (\textit{patience}$=\!20$) sobre la pèrdua de validació del \textit{fold}. L'objectiu
        és $\text{spec}_{\text{mean}}@\text{sens}=1$ amb penalització forta si sens cau de 1.
...
\textbf{Cerca bayesiana.} En lloc d'una graella exhaustiva, s'utilitza el \textit{Tree-structured
Parzen Estimator}~(TPE) sobre $\sim$15 eixos d'optimització (arquitectura, profunditat, pooling, MIL,
hiperparàmetres d'optimitzador). Cada \textit{trial} és una configuració mostrejada pel
\textit{sampler}: els \textbf{40 primers trials} són mostreig uniforme aleatori (exploració inicial),
i a partir del 41è el TPE explota l'historial per concentrar el mostreig a la regió amb millor
\textit{objective}. Tots els mostratges, els 5 \textit{folds} del CV i la inicialització dels pesos
usen \textit{seed} fixa per a reproduïbilitat. L'estudi és persistent (\texttt{SQLite}) i reprenible;
l'espai complet de cerca es recull a l'Annex \ref{app:sweep-config}. \textbf{El conjunt de test
(15\%) no es toca en cap pas del sweep}; només es carrega una vegada, al pas de finalització.
...
\textbf{Objectiu del sweep:} $\text{spec}_{\text{mean}} - 20 \cdot \max(0,\,1 - \text{sens}_{\text{mean}})$,
calculat sobre els 5 \textit{val sets} de la CV. La penalització forta garanteix que els models
amb sensibilitat $<1$ siguin descartats, mentre que entre els que sí mantenen sensibilitat = 1, el
TPE maximitza la mitjana d'especificitat.

...
\textbf{Volum d'experiments.} En conjunt s'han avaluat $N_{\text{base}}=X$
\textit{trials} per a GAT Baseline i $N_{\text{diff}}=X$ per a GAT+DiffPool
($N_{\text{tot}}=X$). La Taula~\ref{tab:winners} resumeix el millor de cada arquitectura
segons el sweep i, després d'aplicar el procediment de finalització
(Secció~\ref{sec:training-scheme}, 3a etapa), les mètriques al conjunt de test.

\begin{table*}[t]
\centering
\caption{Millor configuració per arquitectura segons el sweep bayesià, i mètriques de
validació creuada (CV) i al conjunt de test després del retrain final. Files
ressaltades: \textbf{model seleccionat} per al \textit{reporting} principal (millor entre
les dues branques per l'objectiu del sweep, $\overline{\text{spec}}_{\text{CV}}
- 10\cdot\max(0,1-\overline{\text{sens}}_{\text{CV}})$). El llindar $t^*$ és la mediana
dels 5 llindars obtinguts en cada \textit{fold} CV; \textbf{el test només es toca al final}.}
\label{tab:winners}
\small
...
\midrule
GAT Baseline    & X & X & X & $X \pm X$ & $X \pm X$ & X & X / X \\
GAT+DiffPool    & X & X & X & $X \pm X$ & $X \pm X$ & X & X / X \\
\bottomrule
\end{tabular}\\[2pt]
...
\textbf{Comparació arquitectures.} La Taula~\ref{tab:winners} permet llegir directament
quina arquitectura supera l'altra al test \textbf{[per omplir un cop completat el
\textit{finalize}]}. Els hiperparàmetres òptims trobats per cada branca apareixen també
a l'Annex~\ref{app:best-params}: notablement, les dues branques convergeixen a
combinacions diferents d'$h$, $H$, $\eta$ i $\lambda_{\text{wd}}$ (els eixos compartits
no es barregen entre estudis perquè cada TPE veu només els seus \textit{trials}).

\textbf{Matriu de confusió.} La Taula~\ref{tab:cm} mostra la matriu de confusió del
\textbf{model seleccionat} (l'arquitectura guanyadora segons l'objectiu CV) sobre el test
($n=56$).

\begin{table}[h]
\centering
\caption{Matriu de confusió del \textbf{model seleccionat} sobre el conjunt de test ($n=56$).
Mètriques clíniques: sensibilitat $= X$, especificitat $= X$, VPP $= X$, VPN $= X$.}
\label{tab:cm}
\small
\begin{tabular}{llcc}
\toprule
...
amb importància > 10\,\% en almenys una arquitectura, ja que són els que més empenyen
l'objectiu cap a sensibilitat = 100\,\% amb l'especificitat més alta possible.

\textbf{Resum (a omplir amb els resultats del sweep).} Els eixos més influents observats
són $X$ per a Baseline i $X$ per a DiffPool. La regularització L2 ($\lambda_{\text{wd}}$),
el pes de la classe positiva ($w_+$) i la dimensió oculta ($h$) s'esperen entre els més
rellevants en totes dues branques. Per al DiffPool, el pes del penalty auxiliar
($\lambda_{\text{aux}}$) i el nombre de super-nodes ($K_\ell$) són eixos addicionals
sense paral·lel a la branca Baseline.
...
\textbf{GAT (aquest TFG, llindar $0{,}5$)}          & \textbf{Imatge WSI} & X & X & X & X \\
\textbf{GAT (aquest TFG, llindar $t^*\!=\!X$)$^{\dagger}$} & \textbf{Imatge WSI} & X & X & X & X \\
\bottomrule
\end{tabular}\\[2pt]
...
Figura~\ref{fig:roc-top5} superposa les corbes ROC dels millors candidats de cada arquitectura
amb els punts operatius de les guies clíniques i del model d'IA de referència.
...
(GAT Baseline i GAT+DiffPool). El cercle gros marca el llindar $t^*\!=\!X$ derivat
exclusivament del val set per al model seleccionat (sense \textit{data leakage}). Es marquen
els punts operatius publicats de JSCCR, NCCN, ESMO (quadrats) i LightGBM (diamant). La franja
ressaltada al voltant de Sens.$\,=\!1$ delimita el rang clínicament acceptable: el sistema
busca el punt més a l'esquerra possible dins d'aquesta franja per minimitzar les cirurgies
innecessàries.}
\label{fig:roc-top5}
\end{figure}

L'\textbf{impacte clínic} de l'especificitat al llindar $t^*$ es pot quantificar de manera
directa. La literatura estima que entre el $80\%$ i el $90\%$ de les cirurgies actuals en pT1
amb criteris de risc són innecessàries~\cite{MartinezDeJuan2024}, xifra coherent amb
l'especificitat del $19\%$ de JSCCR. Per cada $100$ pacients N0:

\begin{itemize}\itemsep1pt
  \item \textbf{JSCCR}: $81$ cirurgies innecessàries.
  \item \textbf{NCCN/ESMO}: $48$–$50$ cirurgies innecessàries.
  \item \textbf{GAT (llindar $t^*$)}: $X$ cirurgies innecessàries.
\end{itemize}

Mantenint Sens.$=\!100\%$, el GAT redueix les cirurgies innecessàries en $\sim\!X$ pacients per
cada $100$ N0 respecte de JSCCR i $\sim\!X$ respecte de NCCN/ESMO. Sobre els $40$ N0 del test,
això són $X$ FP en lloc dels $\sim\!32$ que aplicaria JSCCR sobre la mateixa població.
...
\textbf{«GAT Baseline o GAT+DiffPool?»} es respon directament a partir de la
Taula~\ref{tab:winners}: \textbf{el model X obté la millor combinació de
sensibilitat i especificitat al test} (AUC $X$, Sens.\ $X$, Espec.\ $X$). Se'n destaquen
tres observacions:

\begin{itemize}
  \item L'aïllament dels dos estudis bayesians per arquitectura permet una comparació
  neta: cada TPE veu només els seus propis \textit{trials}, evitant que els
  hiperparàmetres compartits ($\eta$, $h$, $H$, \textit{dropout}, etc.) es barregin
  entre branques. L'Annex~\ref{app:best-params} mostra que les dues branques
  convergeixen a combinacions diferents d'aquests eixos.
  \item Sobre el conjunt de dades disponible (367 pacients efectius, 311 al \textit{pool}),
  $X$ supera $X$ en $X$ punts d'AUC al test \textbf{[per omplir]}. Una possible explicació
  és que les seccions tenen pocs centenars de \textit{patches}, una mida petita
  per a què la reducció jeràrquica DiffPool preservi informació útil; alternativament,
  pot ser que el \textit{pooling} jeràrquic aporti avantatge en escenaris amb
  més dades o seccions més grans.
  \item L'agregació MIL i la regularització (L2 + \textit{dropout}) són els factors
  d'influència de primer ordre dins de cada arquitectura, juntament amb el pes de la
  classe positiva $w_+$ que actua sobre el desbalanceig.
\end{itemize}

\subsection{Discussió Comparativa}

L'AUC de test ($X$) es compara amb la de JSCCR ($0{,}596$), NCCN ($0{,}749$) i ESMO
($0{,}743$)~\cite{tanino2024comparative}. L'especificitat ($X$) cal interpretar-la
en el context de les tres guies; la reducció de cirurgies innecessàries derivada
queda quantificada a la Secció~\ref{sec:resultats-comparacio}.

\textbf{VPP i ajust per prevalença.}
El VPP de $X$ s'ha calculat amb la prevalença del test ($28{,}6\%$ N1),
molt superior a la clínica real ($\sim\!8$–$15\%$) \cite{zaffalon2023}. A una
prevalença del $10\%$, el VPP baixa a $X$ i el VPN puja a $X$
...
Al llindar per defecte ($0{,}5$), la sensibilitat del GAT és $X$, inferior a la de
JSCCR/NCCN/ESMO ($98$–$100\%$), però aquestes guies hi arriben amb especificitats
del $19$–$52\%$. Ajustant el llindar a $t^*\!=\!X$ (derivat del val set,
Limitació~4) el GAT assoleix sensibilitat $X$ amb especificitat $X$: una millora
de $X$ punts sobre JSCCR al mateix nivell de sensibilitat.
...
al mateix punt operatiu, arriba a Espec.\ $X$, però amb la imatge WSI com a única
entrada. Els dos models són complementaris: el GAT pot córrer just després
d'escanejar la làmina, sense esperar el report patològic, mentre que el model basat
en \textit{LightGBM} requereix les variables ja anotades.
...
l'AUC CV $X \pm X$ sobre cinc \textit{folds} és la xifra més honesta del rendiment esperat.
...
Per maximitzar la sensibilitat, s'ha buscat el primer punt de la corba ROC del val set
de cada fold del millor model en què TPR$=\!1{,}0$, i s'ha pres la mediana sobre els 5
folds com a $t^*\!=\!X$. Aplicat al test (sense reajustar), dona Sens.\ $X$ amb Espec.\ $X$.
El val set ja s'havia usat per a \textit{early stopping} i selecció del checkpoint, per
la qual cosa $t^*$ pot tenir un biaix optimista lleuger; idealment, hauria de fixar-se
sobre un val set extern.
...
\begin{table}[H]
\centering
\caption{Importància fANOVA per arquitectura. \textbf{Baseline}: $N_{\text{base}}=X$
\textit{trials}. \textbf{DiffPool}: $N_{\text{diff}}=X$. Valors entre 0 (cap influència)
i 1 (l'eix explica tota la variància). \texttt{---}: eix no aplicable a aquella branca.}
...
\textit{Pooling}                          & $X$ & --- \\
$L$ (capes GAT)                           & $X$ & --- \\
$L_{\text{DP}}$ (blocs DiffPool)          & --- & $X$ \\
$K_{\text{top}}$                          & --- & $X$ \\
$K_{\text{bot}}$                          & --- & $X$ \\
$\lambda_{\text{aux}}$                    & --- & $X$ \\
\textit{Diff. readout}                    & --- & $X$ \\
\midrule
\multicolumn{3}{l}{\textit{Eixos compartits (optimitzats independentment a cada study)}} \\[2pt]
$h$ (dim. oculta)                         & $X$ & $X$ \\
$H$ (caps d'atenció)                      & $X$ & $X$ \\
\textit{Dropout}                          & $X$ & $X$ \\
Agregació MIL                             & $X$ & $X$ \\
$\eta$ (\textit{learning rate})           & $X$ & $X$ \\
$\lambda_{\text{wd}}$ (\textit{weight decay}) & $X$ & $X$ \\
$w_+$ (\textit{pos\_weight})              & $X$ & $X$ \\
\textit{Optimizer}                        & $X$ & $X$ \\
\textit{Scheduler}                        & $X$ & $X$ \\
\textit{Grad clipping}                    & $X$ & $X$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textit{Pooling}                          & $X$ & \textit{diff} (fix) \\
$L$ (capes GAT)                           & $X$ & $L_{\text{DP}}+1 = X$ \\
$L_{\text{DP}}$                           & --- & $X$ \\
$K_{\text{top}}, K_{\text{bot}}$          & --- & $X,\,X$ \\
$\lambda_{\text{aux}}$                    & --- & $X$ \\
\textit{Diff. readout}                    & --- & $X$ \\
\midrule
\multicolumn{3}{l}{\textit{Compartits}} \\[2pt]
$h$ / $H$ / \textit{Dropout}              & $X$ / $X$ / $X$ & $X$ / $X$ / $X$ \\
MIL                                       & $X$ & $X$ \\
$\eta$ / $\lambda_{\text{wd}}$ / $w_+$    & $X$ / $X$ / $X$ & $X$ / $X$ / $X$ \\
\textit{Optimizer} / \textit{Scheduler}   & $X$ / $X$ & $X$ / $X$ \\
\midrule
\multicolumn{3}{l}{\textit{Mètriques CV}} \\[2pt]
Objectiu / AUC CV / Sens / Spec           & $X$ / $X$ / $X$ / $X$ & $X$ / $X$ / $X$ / $X$ \\
\bottomrule
\end{tabular}
\end{table}

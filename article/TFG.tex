...
agregació MIL sobre les seccions. Cada arquitectura s'optimitza amb un estudi Optuna
TPE aïllat (304 \textit{trials} per Baseline + 195 per DiffPool); el GAT+DiffPool
guanya al test amb AUC $=0{,}862$ (vs $0{,}755$ del Baseline), sensibilitat $0{,}625$
i especificitat $0{,}850$ al llindar derivat del val set ($n=56$ pacients de test;
367 pacients efectius en total).
\\
\\
...
histological sections. Each architecture is tuned with an isolated Optuna TPE study (304
trials for Baseline + 195 for DiffPool); GAT+DiffPool wins on the test set with AUC $=
0.862$ (vs $0.755$ for Baseline), sensitivity $0.625$ and specificity $0.850$ at the
validation-derived threshold ($n=56$ test patients; 367 effective patients overall).
\\
\\
...
preferible per al diagnòstic N0/N1. Per arribar-hi s'ha definit l'espai d'hiperparàmetres
de l'Annex~\ref{app:sweep-config} (eixos compartits i específics per branca) i s'han
llançat \textbf{dos estudis Optuna separats} per arquitectura, perquè la informació
acumulada en una branca no influeixi la cerca de l'altra. Cada estudi combina una fase
inicial de mostreig aleatori ($\sim$100 \textit{trials} per arquitectura) seguida de
cerca bayesiana (TPE) sobre els \textit{trials} acumulats. Cada \textit{trial} avalua una
configuració amb validació creuada 5-fold sobre el \textit{pool} train+val.

\textbf{Volum d'experiments.} En conjunt s'han avaluat $N_{\text{base}}=350$
\textit{trials} per a GAT Baseline (304 complets, 46 \textit{pruned}) i
$N_{\text{diff}}=231$ per a GAT+DiffPool (195 complets, 36 \textit{pruned}); en total
$N_{\text{tot}}=581$ trials, 499 amb mètriques completes. La Taula~\ref{tab:winners}
resumeix el millor de cada arquitectura segons el sweep i, després d'aplicar el procediment
de finalització (Secció~\ref{sec:training-scheme}, 3a etapa), les mètriques al conjunt de test.

\begin{table*}[t]
\centering
\caption{Millor configuració per arquitectura segons el sweep bayesià, i mètriques de
validació creuada (CV) i al conjunt de test després del retrain final.
\textbf{GAT+DiffPool és el model seleccionat} per a la comparació amb les guies clíniques
(major AUC al test). El llindar $t^*$ és la mediana dels 5 llindars obtinguts en cada
\textit{fold} CV; \textbf{el test només es toca al final}. Cap dels dos models manté
sens.\ $=100\%$ al test, contràriament al que s'observa a la CV (vegeu §\ref{sec:limitacions}).}
\label{tab:winners}
\small
\begin{tabular}{lccccccc}
\toprule
\textbf{Arquitectura} & \textbf{$N_{\text{trials}}$}
  & \textbf{Pooling} & \textbf{MIL}
  & \textbf{Obj. CV} & \textbf{AUC CV} & \textbf{AUC test} & \textbf{Sens.\ / Espec.\ test} \\
\midrule
GAT Baseline       & 350 & \textit{mean\_max} & \textit{attention} & $0{,}530$ & $0{,}867$ & $0{,}755$ & $0{,}562 / 0{,}825$ \\
\textbf{GAT+DiffPool}      & \textbf{231} & \textbf{\textit{diff}} & \textbf{\textit{mean}}  & $\mathbf{0{,}477}$ & $\mathbf{0{,}855}$ & $\mathbf{0{,}862}$ & $\mathbf{0{,}625 / 0{,}850}$ \\
\bottomrule
\end{tabular}\\[2pt]
\footnotesize{Objectiu del sweep: $\overline{\text{spec}}_{\text{CV}} - 10\cdot\max(0,
1 - \overline{\text{sens}}_{\text{CV}})$. Vegeu Annex~\ref{app:sweep-config}.}
\end{table*}

\textbf{Comparació arquitectures.} La Taula~\ref{tab:winners} mostra un resultat
interessant: tot i que el \textbf{GAT Baseline} surt millor a la CV ($0{,}530$ vs
$0{,}477$ d'objectiu), al \textbf{conjunt de test} és el \textbf{GAT+DiffPool} qui
obté el millor AUC ($0{,}862$ vs $0{,}755$, una diferència d'$11$ punts).
Això suggereix que el GAT Baseline ha sobreajustat el \textit{val set} durant la cerca
bayesiana, mentre que el GAT+DiffPool—amb la reducció jeràrquica del graf actuant com
a regularitzador estructural—generalitza millor al test. Els hiperparàmetres òptims
trobats per cada branca apareixen a l'Annex~\ref{app:best-params}: les dues convergeixen
a $h=128$ però difereixen substancialment en MIL (\textit{attention} vs \textit{mean}),
$H$ ($4$ vs $2$), $\eta$ ($1{,}1\cdot10^{-3}$ vs $5{,}3\cdot10^{-4}$) i $w_+$ ($6{,}77$ vs
$3{,}52$). Els eixos compartits no es barregen entre estudis perquè cada TPE veu només
els seus \textit{trials}.

\textbf{Matriu de confusió.} La Taula~\ref{tab:cm} mostra la matriu de confusió del model
seleccionat (GAT+DiffPool) sobre el test ($n=56$, $16$ N1 + $40$ N0).

\begin{table}[h]
\centering
\caption{Matriu de confusió del \textbf{GAT+DiffPool} (model seleccionat) sobre el conjunt
de test ($n=56$) al llindar $t^*=0{,}299$.
Mètriques clíniques: sensibilitat $=0{,}625$, especificitat $=0{,}850$, VPP $=0{,}625$, VPN $=0{,}850$.}
\label{tab:cm}
\small
\begin{tabular}{llcc}
\toprule
...
($I=0{,}64$): \textit{attention} és la millor i els altres modes (\textit{mean},
\textit{noisy-OR}, \textit{max}) queden força per sota. Cap altre eix supera el $10\%$;
els segons són \textit{pooling} ($0{,}08$), $w_+$ ($0{,}07$) i \textit{dropout} ($0{,}05$).
...
$\eta$ ($0{,}13$), $\lambda_{\text{wd}}$ ($0{,}08$) i \textit{dropout} ($0{,}08$). Els
eixos específics de DiffPool ($L_{\text{DP}}$, $K_{\text{top}}/K_{\text{bot}}$,
\textit{diff readout}) tenen importància moderada però conjunta significativa ($>0{,}15$).

\subsection{Comparació amb guies clíniques i SOTA}
\label{sec:resultats-comparacio}

La Taula~\ref{tab:comparison} situa el \textbf{model seleccionat} (la branca
guanyadora de la Taula~\ref{tab:winners}) davant de les guies clíniques de
...
\textbf{GAT+DiffPool (aquest TFG, llindar $0{,}5$)}          & \textbf{Imatge WSI} & $56{,}2\%$ & $95{,}0\%$ & $0{,}862$ & $75{,}6\%$ \\
\textbf{GAT+DiffPool (aquest TFG, llindar $t^*\!=\!0{,}299$)$^{\dagger}$} & \textbf{Imatge WSI} & $62{,}5\%$ & $85{,}0\%$ & $0{,}862$ & $73{,}8\%$ \\
\bottomrule
\end{tabular}\\[2pt]
\footnotesize{$^{\dagger}$\,$t^*$ s'ha determinat com a mediana dels 5 llindars dels \textit{val sets}
dels folds CV (sens=$100\%$ en cada fold), sense \textit{data leakage} del test.
Vegeu §\ref{sec:limitacions} sobre la caiguda de la sensibilitat al test.}
\end{table*}

En la detecció de metàstasi, un fals negatiu (no tractar un pacient N1) és un error inacceptable,
mentre que un fals positiu (cirurgia innecessària) és recuperable. La Figura~\ref{fig:roc-top5}
superposa les corbes ROC dels millors candidats de cada arquitectura amb els punts operatius
de les guies clíniques i del model d'IA de referència.

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{img/roc_top5.pdf}
\caption{Corbes ROC al conjunt de test (56 pacients) dels millors models de cada arquitectura
(GAT Baseline, AUC $=0{,}755$; GAT+DiffPool, AUC $=0{,}862$). El cercle gros marca el
llindar $t^*\!=\!0{,}299$ del model seleccionat (DiffPool), derivat exclusivament del val
set (sens=$100\%$ a cada fold CV) i sense \textit{data leakage} del test. Es marquen els
punts operatius publicats de JSCCR, NCCN, ESMO (quadrats) i LightGBM (diamant).}
\label{fig:roc-top5}
\end{figure}

\textbf{Impacte clínic.} El llindar $t^*$ deriva del val set amb sens=$100\%$ a cada fold,
però \textbf{al test la sensibilitat cau al $62{,}5\%$} (6 N1 no detectats sobre 16,
matriu confusió de la Taula~\ref{tab:cm}). En aquest punt operatiu, l'especificitat
($85{,}0\%$) és molt millor que la de les guies clíniques, però el cost humà d'un fals negatiu
fa que la comparació justa sigui per AUC i no per un punt operatiu mantenint sens=$100\%$
(que cap dels models GAT aconsegueix al test). Vegeu §\ref{sec:limitacions} (punt 4) per a la
discussió detallada de la caiguda de sensibilitat.

Si s'apliqués el llindar baix ($0{,}5$), l'especificitat puja al $95{,}0\%$ però la sensibilitat
queda al $56{,}2\%$. Per a una eina de cribratge clínic, el rendiment actual no és suficient:
caldria una sensibilitat $\geq 95\%$ que el GAT a aquesta mida de dades no aconsegueix sense
una caiguda dràstica de l'especificitat.
...
l'entrenament del model GAT amb dos estudis Optuna aïllats (un per arquitectura) i
l'avaluació final sobre un conjunt de test fix (56 pacients). La pregunta central
\textbf{«GAT Baseline o GAT+DiffPool?»} es respon a partir de la Taula~\ref{tab:winners}:
\textbf{el GAT+DiffPool obté el millor rendiment al test} (AUC $0{,}862$, Sens.\ $62{,}5\%$,
Espec.\ $85{,}0\%$), $11$ punts d'AUC per sobre del GAT Baseline ($0{,}755$). Se'n destaquen
tres observacions:

\begin{itemize}
  \item \textbf{Resultat sorprenent del CV vs test}: el GAT Baseline és millor a la CV
  (objectiu $0{,}530$ vs $0{,}477$), però el GAT+DiffPool guanya al test. Això suggereix
  que el Baseline ha sobreajustat el \textit{val set} durant la cerca bayesiana, mentre
  que el DiffPool generalitza millor al test (la reducció jeràrquica a super-nodes actua
  com a regularitzador estructural). Aquest mecanisme—pèrdua auxiliar
  $\mathcal{L}_{\text{LP}}+\mathcal{L}_{\text{E}}$, més la compressió del graf—té
  l'efecte de \emph{limitar la capacitat de memoritzar} el val set, i el premi és una
  millor projecció al test.
  \item \textbf{Aïllament dels estudis}: cada TPE veu només els seus propis \textit{trials},
  evitant que els hiperparàmetres compartits ($\eta$, $h$, $H$, \textit{dropout}, etc.)
  es barregin entre branques. L'Annex~\ref{app:best-params} mostra que les dues branques
  convergeixen a hidden $h=128$ però difereixen substancialment en MIL (\textit{attention}
  vs \textit{mean}), heads i \textit{learning rate}.
  \item \textbf{Factors d'influència}: l'agregació MIL és el factor més influent a totes
  dues branques (importància fANOVA $0{,}64$ a Baseline, $0{,}22$ a DiffPool). A DiffPool,
  el pes auxiliar $\lambda_{\text{aux}}$ i el \textit{learning rate} prenen un paper
  central ($\sim 0{,}13$–$0{,}14$ cadascun).
\end{itemize}

\subsection{Discussió Comparativa}

L'AUC de test del GAT+DiffPool ($0{,}862$) supera la de JSCCR ($0{,}596$), NCCN ($0{,}749$)
i ESMO ($0{,}743$)~\cite{tanino2024comparative}. Tanmateix, la comparació amb les guies en
els punts operatius respectius no és tan favorable: les guies mantenen sens=$98$–$100\%$
mentre que el GAT+DiffPool només arriba a sens=$62{,}5\%$ ($t^*$) o $56{,}2\%$ ($0{,}5$).

\textbf{VPP i ajust per prevalença.}
El VPP de $62{,}5\%$ s'ha calculat amb la prevalença del test ($28{,}6\%$ N1),
superior a la clínica real ($\sim\!8$–$15\%$) \cite{zaffalon2023}. A una
prevalença del $10\%$, el VPP baixa a $\sim\!32\%$ i el VPN puja a $\sim\!95\%$
(càlcul per Bayes amb les Sens./Espec.\ observades: $0{,}625/0{,}850$).
Sensibilitat i especificitat, independents de la prevalença, segueixen sent les
mètriques comparatives més robustes.

\textbf{Sensibilitat insuficient per a ús clínic.}
Al llindar derivat del val set ($t^*\!=\!0{,}299$), la sensibilitat al test és del
$62{,}5\%$ ($10/16$ N1 detectats; 6 fals negatius). Això està molt per sota del rang
clínicament acceptable ($\geq\!95\%$ per a una eina de cribratge). El llindar del val
set garanteix sens=$100\%$ al \textit{val} de cada fold, però aquesta condició no es
manté al test, probablement per la combinació de mida petita del dataset ($n=56$ al
test, només $16$ N1) i la variància entre splits.
...
queda 10 punts per sota en AUC ($0{,}862$) i, més importantment, molt per sota en
sensibilitat al test. Els dos models són complementaris en termes d'entrada: el GAT
podria córrer just després d'escanejar la làmina, sense esperar el report patològic;
LightGBM requereix les variables ja anotades. Però amb les dades actuals, el GAT no
ha demostrat ser una alternativa viable per a ús clínic autònom.

\subsection{Limitacions}
\label{sec:limitacions}

\textbf{(1) Mida del test.}
Amb $56$ pacients al test (només $16$ N1), els intervals de confiança al $95\%$ de l'AUC són
amplis. L'AUC CV del millor DiffPool és $0{,}855$, propera a l'AUC test ($0{,}862$), però la
variància CV de l'especificitat és alta (mitjana $0{,}477\pm 0{,}110$), reflectint la
inestabilitat entre folds. Caldria un test més gran per a conclusions clíniques robustes.
...
\textbf{(4) Selecció del llindar i caiguda de la sensibilitat al test.}
Per maximitzar la sensibilitat, s'ha buscat el primer punt de la corba ROC del val set de
cada fold del millor model en què TPR$=\!1{,}0$, i s'ha pres la mediana sobre els 5 folds
com a $t^*\!=\!0{,}299$ per al DiffPool. Aplicat al test, dona Sens.\ $62{,}5\%$ i
Espec.\ $85{,}0\%$—lluny del $100\%$ de sensibilitat que es garantia per construcció a
cada val set. Tres causes possibles, no excloents: (a) el val set ja s'usa per a
\textit{early stopping}, donant un biaix lleugerament optimista al threshold; (b) la
variància entre folds ($\sigma_{t^*}\!=\!0{,}17$, rang $[0{,}03,\,0{,}49]$) indica que la
distribució de probabilitats N1 canvia significativament entre splits; (c) la mida del
test ($16$ N1) amplifica l'error per a qualsevol llindar fix. Per a un sistema clínicament
útil cal validació externa i, probablement, una estratègia de calibració de llindar més
robusta (e.g., \textit{conformal prediction} o \textit{ensemble} de top-K trials).
...
\subsection{Millors hiperparàmetres per arquitectura}
\label{app:best-params}

La Taula~\ref{tab:best-params} mostra la configuració amb millor objectiu CV per a cada
estudi (Baseline i DiffPool). Els eixos compartits s'optimitzen per separat per cada
branca, per la qual cosa poden convergir a valors diferents. Aquests són els
hiperparàmetres usats al \texttt{--finalize} per generar els resultats al test
(Taula~\ref{tab:winners}).

\begin{table}[H]
\centering
\caption{Millor configuració per arquitectura segons el sweep bayesià.}
\label{tab:best-params}
\small
\begin{tabular}{lcc}
\toprule
\textbf{Eix} & \textbf{Baseline (\#303)} & \textbf{DiffPool (\#199)} \\
\midrule
\multicolumn{3}{l}{\textit{Arquitectura}} \\[2pt]
\textit{Pooling}                          & \textit{mean\_max} & \textit{diff} (fix) \\
$L$ (capes GAT)                           & $3$ & $3$ (= $L_{\text{DP}}+1$) \\
$L_{\text{DP}}$                           & --- & $2$ \\
$K_{\text{top}}, K_{\text{bot}}$          & --- & $50,\,3$ \\
$\lambda_{\text{aux}}$                    & --- & $0{,}108$ \\
\textit{Diff. readout}                    & --- & \textit{mean\_max} \\
\midrule
\multicolumn{3}{l}{\textit{Compartits}} \\[2pt]
$h$ / $H$ / \textit{Dropout}              & $128$ / $4$ / $0{,}355$ & $128$ / $2$ / $0{,}437$ \\
MIL                                       & \textit{attention} & \textit{mean} \\
$\eta$ / $\lambda_{\text{wd}}$ / $w_+$    & $1{,}1\!\cdot\!10^{-3}$ / $6{,}6\!\cdot\!10^{-3}$ / $6{,}77$ & $5{,}3\!\cdot\!10^{-4}$ / $5{,}5\!\cdot\!10^{-3}$ / $3{,}52$ \\
\textit{Optimizer} / \textit{Scheduler}   & \textit{adam} / \textit{cosine} & \textit{adam} / \textit{cosine} \\
\midrule
\multicolumn{3}{l}{\textit{Mètriques CV}} \\[2pt]
Objectiu / AUC CV / Sens / Spec           & $0{,}530$ / $0{,}867$ / $1{,}00$ / $0{,}530$ & $0{,}477$ / $0{,}855$ / $1{,}00$ / $0{,}477$ \\
\bottomrule
\end{tabular}
\end{table}

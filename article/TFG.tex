...
\textbf{Cerca bayesiana.} En lloc d'una graella exhaustiva, s'utilitza \textit{Tree-structured Parzen
Estimator}~(TPE) sobre $\sim$15 eixos d'optimització (arquitectura, profunditat, pooling, MIL,
hiperparàmetres d'optimitzador). L'estudi és persistent (\texttt{SQLite}) i reprenible, amb pruner
de mediana per descartar \textit{trials} clarament pitjors. El detall complet dels eixos i rangs es
recull a l'Annex~\ref{app:sweep-config}. Cada \textit{trial} desa el millor checkpoint per
\textit{fold}, juntament amb les probabilitats al val, perquè el frontend pugui carregar-los i
permetre l'anàlisi posterior.

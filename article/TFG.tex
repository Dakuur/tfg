...
intercalat ($N \to \cdots \to K_{L_{\text{DP}}}$ super-nodes) per
...
TPE aïllats per arquitectura (5-fold CV + retrain
90/10 de les millors configuracions obtingudes). El millor model és un \textbf{GAT
...
with $L_{\text{DP}}$ DiffPool blocks interleaved between GAT layers ($N \to \cdots \to K_{L_{\text{DP}}}$ super-nodes) for
...
(one per architecture) (5-fold CV + 90/10 retraining of
the best configurations). The best model is a \textbf{GAT Baseline}, achieving test
...
DiffPool (millor)       & DiffPool & diff/mean     & $128/2/0{,}44$ & $0{,}847\!\pm\!0{,}048$ & $0{,}786$ & $0{,}63\,/\,0{,}700$ & $0{,}500\,/\,0{,}825$ \\
\bottomrule
\end{tabular}\\[2pt]
...
apareixen com a \texttt{---} a l'altra columna. Tots els valors provenen de l'anàlisi fANOVA calculada per Optuna sobre els \textit{trials} complets de cada estudi.
...
Llindar $t^*$           & post-CV            & sobre la corba ROC del val, condició TPR$\geq 0{,}95$    & determinista \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Objectiu del sweep:}
$\text{objective} = \overline{\text{spec}}_{\text{CV}} - 10 \cdot \max(0,\; 0{,}95 - \overline{\text{sens}}_{\text{CV}})$,
...
(llindar més alt amb TPR$\geq 0{,}95$). Aquesta és l'\textbf{única} mètrica que el TPE veu per decidir

...
\node[draw, dashed, rounded corners=2pt, fill=red!4, font=\sffamily\scriptsize, align=left,
      ...
      (h-mod) {$L,\ h,\ H,\ p_{\text{drop}}$,\\\textit{readout},
...
\end{tikzpicture}
\caption{Diagrama de flux del \textit{pipeline} complet amb els hiperparàmetres optimitzats
(columna dreta, caixes discontínues). \textit{Blocs grisos}: etapes fetes pels companys de
l'equip i dades de referència; \textit{blocs blaus}: contribucions d'aquest TFG. La fletxa
${\times}S$ indica que les $S$ seccions d'un pacient es processen com a grafs independents i
s'agreguen via MIL per al diagnòstic per pacient.}

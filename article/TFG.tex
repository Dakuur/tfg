...
\begin{figure*}[h]
    ...
    intercalades), que col·lapsen progressivament el graf fins al dos super-nodes finals, en aquest cas.}
    \label{fig-diffpool}
\end{figure*}
...
\begin{table*}[t]
\centering
\caption{Comparació de configuracions re-avaluades amb el procediment estricte de la
Secció~\ref{sec:training-scheme} (5-fold CV + retrain 90/10 + test únic). \textbf{Files ressaltades}: model
...
\textbf{Lectura dels resultats.} El sweep TPE no supera el grid recuperat
...
mantenen sens.\ $=100\%$ al test amb el llindar $t^*$, cosa que cap del sweep
aconsegueix. Finalment, el GAT Baseline supera GAT+DiffPool en aquest dataset:
...
Taula~\ref{tab:configs-compare}) davant de les guies clíniques de referència (Secció~\ref{sec:guidelines-sota}) i de dos models d'IA de referència (un sobre variables clíniques i un altre sobre imatge WSI).
...
Model: Grid \#1 (vegeu Taula~\ref{tab:configs-compare}).}
\end{table*}
...
set sense \textit{data leakage}): $t^*\!=\!0{,}133$ per al Baseline
...
Una comparació més justa, a la \textbf{mateixa modalitat}, és la de Song et
al.~\cite{song2024lnm}, un model MIL d'atenció sobre WSI H\&E (sense variables clíniques),
del mateix paradigma que el GAT però amb un extractor CNN per \textit{patch} en lloc d'un
graf. Sobre $1\,281$ pacients obté AUC $0{,}781$, sens.\ $92{,}9\%$ i espec.\ $57{,}6\%$. A
una especificitat pràcticament idèntica ($57{,}5\%$ vs $57{,}6\%$), el GAT assoleix
sens.\ $100\%$ (vs $92{,}9\%$) i un AUC superior ($0{,}875$ vs $0{,}781$); cal matisar, però,
que la seva avaluació és molt més robusta estadísticament ($1\,281$ pacients vs els $56$ del
nostre test). Aquest resultat suggereix que representar la secció com a graf espacial és, com a
mínim, competitiu amb l'agregació MIL plana sobre \textit{patches}.
